...
| T0.6 | Setup Jamendo + Freesound client | **SELESAI** | lib/music/jamendo.ts + lib/music/freesound.ts + lib/actions/music.ts. Filter lisensi CC0/CC-BY/CC-BY-SA (tolak NC/ND). Limit dinaikkan 20→50 + sort by popularity_total (Jamendo) / rating_desc (Freesound). Terverifikasi: Jamendo "ambient" 1→6 aman, Freesound "whoosh" 6/20→44/50 aman. |
| — | MusicPicker UI + attach ke scene | **SELESAI** | components/MusicPicker.tsx (modal search + preview + attach). Terintegrasi di production detail page tiap scene card. Kolom name ditambahkan ke music_tracks. |
| — | Export ZIP per production | **SELESAI** | /api/productions/[id]/export-zip dengan auth manual. ZIP berisi folder per-scene (prompt, narasi, audio, musik + lisensi). Error handling individual per file. Tombol EXPORT ZIP di production page. |
